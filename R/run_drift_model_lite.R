...
run_drift_model_lite <- function(project_folder, input_data, executable_source, save_file = NULL, return_results = T, keep_folder = T, tries = 5) {
  # define header
	...
	writeLines(paste0(c(sprintf(c("%-25s", rep("%-40s",input_data[[4]][4])), c("landscape_file_name:", input_data[[5]]))), collapse = ""), con = con_file, useBytes = F)
	close(con_file)
