...
run_drift_model <- function(project_folder, input_data, executable_source, parameter = NULL, n_thread = NULL, save_file = NULL, return_results = T, keep_folder = T){
  ...
  output <- foreach(i_run = 1:n_run) %dopar% {
    # define header
    header <- list(application_input = data.frame(tractor_speed = "[m/s]",boom_width = "[m]",boom_height = "[m]",nozzle_angle = "[°]",application_pres = "[kPa]",app_rate_mh = "[m³/h]",app_rate_mha = "[m³/ha]",app_rate_kgha = "[kg/ha]",sol_concentration = "[kg/m³]",AI_density = "[kg/m³]",AI_molar_mass = "[kg/mol]",AI_vapor_pressure = "[Pa]",AI_diffusion_coef = "[m²/s]",AI_diffusion_temp = "[°C]",swath_number = "[-]"),
    ...
    for(i in 1:nrow(run_input[[1]])){writeLines(paste0(c(sprintf(c(rep("%20.2f",ncol(run_input[[1]]))),run_input[[1]][i,])), collapse = ""), useBytes = F)}
    sink()
    ...
    for(i in 1:nrow(run_input[[2]])){writeLines(paste0(c(sprintf(c("%20.8f","%20.8f"),run_input[[2]][i,])), collapse = ""), useBytes = F)}
    sink()
    ...
    for(i in 1:nrow(run_input[[3]])){writeLines(paste0(c(sprintf(c(rep("%20.2f",ncol(run_input[[3]]))),run_input[[3]][i,])), collapse = ""), useBytes = F)}
    sink()
    ...
    for(i in 1:nrow(run_input[[4]])){writeLines(paste0(c(sprintf(c(rep("%20.2f",ncol(run_input[[4]]))),run_input[[4]][i,])), collapse = ""), useBytes = F)}
    sink()
  ...
  # restructure results
  if(!is.null(parameter)){
    result <- list(parameters = parameter,
                   output = output)
  ...
  if(!is.null(save_file)){rlist::list.save(result,paste0(project_folder,"/",save_file,".rdata"))}

  # return results
  if(return_results){return(result)}
